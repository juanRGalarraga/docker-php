...
* @param pointer $res default null Opcional. Puntero a un conjunto de resultados. Si se proporciona, el método se refiere a ese resultado, si no, usa el puntero local al objeto establecido en la última llamada a ->Query()
...
/**
* Summary. Analiza el resultado de una consulta y devuelve una lista con los nombres de los campos involucrados en esa consulta.
* @param pointer $res default null Opcional. Puntero a un conjunto de resultados. Si se proporciona, el método se refiere a ese resultado, si no, usa el puntero local al objeto establecido en la última llamada a ->Query()
* @param boolean $ordinal_only default false Determina que solo se devuelvan los nombres de campos con tipo de dato "ordenable".
* @return bool/array Un array con la lista de nombres de campos o false en caso de error.
*/
	public function GetFieldsNames($res = null, $ordinal_only = false) {
		$result = false;
		if (is_null($res)) { $res = $this->result; }
		$fieldCount = mysqli_num_fields($res);
		for ($i=0; $i < $fieldCount; $i++) { 
			$field = mysqli_fetch_field_direct($res, $i); // Esto regresa un objeto, no un array.
			if ($ordinal_only) {
				if (!in_array($field->type, [MYSQLI_TYPE_TINY_BLOB, MYSQLI_TYPE_MEDIUM_BLOB, MYSQLI_TYPE_LONG_BLOB, MYSQLI_TYPE_BLOB, MYSQLI_TYPE_JSON])) {
					$result[] = $field->name;
				} else { continue; }
			} else {
				$result[] = $field->name;
