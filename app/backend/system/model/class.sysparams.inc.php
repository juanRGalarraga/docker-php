...
*/
	public function Get($parametro=null, $default = null) {
		...
		try {
			if (!empty($parametro)) {
				$sql = "SELECT * FROM ".SQLQuote(self::tabla_parametros). " WHERE LOWER(`nombre`) = LOWER('".$this->RealEscape(mb_substr($parametro,0,64))."') ";
				$this->Query($sql);
				if ($fila = $this->First()) {
...
*/
	public function GetById($id) {
		...
		try {
			...
			$sql = "SELECT * FROM ".SQLQuote(self::tabla_parametros)." WHERE `id`=$id";
			// if ($this->estricto) {
			// 	$sql .= "AND `estado` = 'HAB'";
			// }
			$this->Query($sql);
...
*/
	public function UpdateReg($id, $reg) {
		...
		try {
			...
			$reg = $this->RealEscapeArray($reg);
			$this->Update(self::tabla_parametros, $reg, "`id` = ".$id);
			$result = true;
		} catch(Exception $e) {
			$this->SetError($e);
		}
...
*/
	...
	/**
	 * Genera una consulta para armar un listado utilizando los filtros que se requieran
	 * @param array $data - Arreglo que contendrá como key el tipo de filtro y value como el filtro a aplicar
	 * @return boolean/array $result - False o un array en caso de que se encuentren resultados. 
	 */

	public function GetListado($data){
		$result = false;
		$limit = 25;
		$pag = 1;
		$orderBy = '';
		...
		try {
			...
			if(isset($data['limit']) && SecureInt($data['limit'])){
				$limit = $data['limit'];
			}

			if(isset($data['pag']) && SecureInt($data['pag'])){
				$pag = $data['pag'];
			}

			$select = "SELECT `parametros`.*, `grupos`.`nombre` AS `nombre_grupo` ";

			$from = "FROM ".SQLQuote(self::tabla_parametros)." AS `parametros` LEFT JOIN ".SQLQuote(self::tabla_parametros_grupos)." as grupos ON grupos.id = `parametros`.`grupo_id` ";
			
			if (isset($data['estado']) && CanUseArray($data['estado'])) {
				$estado = array();
				foreach ($data['estado'] as $value) {
					if(array_key_exists($value, ESTADOS_VALIDOS)){
						$estado[] = $value;
					}
				}
				if(CanUseArray($estado)){
					$where .= " AND (`parametros`.`estado` IN ('" . implode("','", $estado) . "')) ";
				}
			}
			
			if (isset($data['grupos']) && CanUseArray($data['grupos'])) {
				$grupos = array();
				foreach ($data['grupos'] as $value) {
					if(SecureInt($value, false)){
						$grupos[] = $value;
					}
				}
				if(CanUseArray($grupos)){
					$where .= " AND (`parametros`.`grupo_id` IN ('" . implode("','", $grupos) . "')) ";
				}
			}
			
			if (!empty($data['buscar'])){
				$busqueda = $this->RealEscape($data['buscar']);
				$where .= "AND (`grupos`.`nombre` REGEXP '".$busqueda."') 
							OR (`parametros`.`nombre` REGEXP '".$busqueda."') ";
			}

			if(isset($data['ordenes']) && CanUseArray($data['ordenes'])){
				$orderBy = " ORDER BY ".self::MakeOrderBy($data['ordenes']);
			} else {
				$orderBy = " ORDER BY `parametros`.`fechahora_modif` DESC";
			}
			
			$limit = " LIMIT ".(($limit*$pag)-$limit).",".($limit*$pag);
	...
	/**
	 * Prepara la consulta Order By para ejecutar una consulta SQL
	 * @param array $ordenes - Clave = "nombre_del_campo" Valor = "ASC" o "DESC"
	*/
	private static function MakeOrderBy($ordenes) {
		$result = '';
		foreach ($ordenes as $campo => $orden) {
			if(in_array($orden, array('ASC', 'DESC'))){
				$p = explode('>',$campo);
				if (count($p) > 1) {
	...
	*/
	public function GetGrupos() {
		$result = false;
		try {
			$sql = "SELECT * FROM ".SQLQuote(self::tabla_parametros_grupos)." ORDER BY `nombre` AND `estado`='HAB'";
			$this->Query($sql);
			if ($fila = $this->First()) {
				do {
					$result[] = $fila;
...
*/
	public function GetGrupo($id) {
		...
		try {
			...
			$sql = "SELECT * FROM ".SQLQuote(self::tabla_parametros_grupos)." WHERE id=$id";
			$this->Query($sql);
			if ($fila = $this->First()) {
				$result = $fila;
				$this->existe = true;
...
*/
	public function GetGrupoByName(string $nombre):?object {
		...
		try {
			...
			$sql = "SELECT * FROM ".SQLQuote(self::tabla_parametros_grupos)." WHERE LOWER(`nombre`) LIKE LOWER('".$this->RealEscape($nombre)."')";
			$this->Query($sql);
			if ($fila = $this->First()) {
...
*/
	public function GetAllByGrupo(int $grupo_id):array {
		...
		try {
			...
			$sql = "SELECT * FROM ".SQLQuote(self::tabla_parametros)." WHERE `grupo_id` = ".$grupo_id." AND `estado` = 'HAB'";
			$this->Query($sql);
			if ($fila = $this->First()) {
				do {
