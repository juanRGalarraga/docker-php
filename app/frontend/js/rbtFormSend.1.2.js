...
var rbtFormSend = function (theForm, param) {
	...
	this.defaultOptions = {
		url: 'localhost', // Dónde le pega el ajax?
	...
	this.xhr.onreadystatechange = function (ev) { // ev es el objeto evento.
		switch (theSender.xhr.readyState) {
			...
			case 4: // La petición se completó. Esto es lo que nos interesa.
				...
				theSender.options.onFinish.call(theSender, theSender.xhr.status, theSender.xhr.statusText, theSender.xhr.responseText, theSender.objResponse);
				break;
		}
	}
	...
	this.Send = function (formulario, param) {
		...
		theSender.xhr.open(theSender.options.method || 'POST', theSender.options.url || 'localhost');
	...
	this.addInput = function (theFile) { // Poner la cartita con los datos del archivo dropeado y además el input file.
		...
		theFileSize.innerHTML = theSender.formatFileSizes(theFile.size,2);
	...
	/* Helpers functions */
	this.formatFileSizes = function(size, decimals = 2) {
