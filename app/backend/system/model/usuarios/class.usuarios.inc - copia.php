...
class cUsuarios extends cModels {
	...
	private $tabla_persona = TBL_backend_personas;
	...
	public function __construct() {
		...
		$this->mainTable = $this->tabla_usuarios;
	}
	/**
	* Obtiene el nombre completo del usuario
	*/
	public function GetCompleteName(int $id = null) :? string{
		$result = null;
		try {
			if (!SecureInt($id)) { throw new Exception(__LINE__." ID debe ser un número."); }
			$sql = "SELECT CONCAT(`nombre`, ' ', `apellido`) AS nombre_completo FROM ".SQLQuote($this->tabla_usuarios)." WHERE `id`=$id";
			$this->Query($sql);
			if ($fila = $this->First()) {
				$result = $fila['nombre_completo'];
			}
		} catch (Exception $e) {
			$this->SetError($e);
		}
	...
	public function Get():?object {
		...
		try {
			...
			$this->sql = "SELECT `user`.*,`persona`.`nombre` AS `persona_nombre`,`persona`.`apellido` AS `persona_apellido` FROM ".SQLQuote($this->tabla_usuarios)." AS `user` INNER JOIN ".SQLQuote($this->tabla_persona)." AS `persona` ON `persona`.`id`=`user`.`persona_id` WHERE `user`.`id` = ".$this->id;
...
/**
* Summary. Cambia o agrega las opciones del usuario.
*/
	public function SetOptions($data, $usuario_id = 1) {
		$result = false;
		try {
			if (!$this->existe) { return false; }
			$data = (array)$data;
			if (!CanUseArray($data)) { return true; }
			
			$opciones = (array)$this->opciones;
			
			$reg = array(
				'opciones'=>json_encode(array_merge($opciones, $data), JSON_FORCE_OBJECT),
				'fecha_modif'=>cFechas::Ahora(),
...
*/
	public function GetByUsername($username) {
		...
		try {
			...
			$sql = "SELECT `user`.*,`persona`.`nombre` AS `persona_nombre` ,`persona`.`apellido` AS `persona_apellido` FROM ".SQLQuote($this->tabla_usuarios)." AS `user` INNER JOIN ".SQLQuote($this->tabla_persona)." AS `persona` ON `persona`.`id`=`user`.`persona_id` WHERE LOWER(`username`) = LOWER('".$username."') AND `estado` != 'ELI' LIMIT 1;";
			if ($this->FirstQuery($sql)) {
				$this->SetUserData();
				$result = true;
			}
		} catch (Exception $e) {
...
*/
	public function CheckLogin() {
		...
		try {
			if ((isset($_SESSION[$this->session_name])) and ($_SESSION[$this->session_name] > 0)) {
				...
				if ($this->session_record = $this->FirstQuery($sql)) {
					$this->id = $this->session_record->usuario_id;
					$this->Get();
					...
					//if ($fila['ip'] == GetIP()) { // Es la misma IP?
						if (($this->session_record->navegador == $_SERVER['HTTP_USER_AGENT'])) { // Está en el mismo navegador?
							if ((date('U') - $this->session_record->idle) < ((isset($this->tsession))?$this->tsession:3600)) { // La sesión no se venció?
								...
								cLogging::Write(__FILE__ ." ". __LINE__ ." La sesión del usuario expiró o tsession no está definido.");
							}
						} else {
							cLogging::Write(__FILE__ ." ". __LINE__ ." El usuario no está en el mismo navegador que en la sesión anterior.");
						}
					/*} else {
	...
	public function UpdateSession() {
		...
		try {
			...
			$this->Update($this->tabla_sesiones,['idle'=>date("U")]," `id` =".$idSesion);
...
*/
	...
	*/
	public function GetActivos($user_nivel = []) {
		$result = false;
		try {
			$sql = "SELECT `user`.*,`persona`.`nombre` AS `persona_nombre` ,`persona`.`apellido` AS `persona_apellido` FROM ".SQLQuote($this->tabla_usuarios)." AS `user` INNER JOIN ".SQLQuote($this->tabla_persona)." AS `persona` ON `persona`.`id`=`user`.`persona_id` WHERE estado = 'HAB'";
	...
	*/
	public function GetByMarca($firma) {
		$result = false;
		try {
			$sql = "SELECT `user`.*,`persona`.`nombre` AS `persona_nombre` ,`persona`.`apellido` AS `persona_apellido` FROM ".SQLQuote($this->tabla_usuarios)." AS `user` INNER JOIN ".SQLQuote($this->tabla_persona)." AS `persona` ON `persona`.`id`=`user`.`persona_id` WHERE marca_id = '".$firma."' AND estado = 'HAB'";
			$this->Query($sql);
			if ($this->numrows > 0) {	
          		$result = array();
          		while($fila = $this->Next()){
...
*/
	...
	public function SetUserData() { // Pone los datos del usuario en el objeto a partir del registro.
		...
		$this->nombre_apellido = @$this->persona_nombre." ".@$this->persona_apellido;
		$this->NomApe = @$this->persona_nombre." ".@$this->persona_apellido;
		$this->iniciales = substr(@$this->persona_nombre,0,1)." ".substr(@$this->persona_apellido,0,1);
		$this->nivel_int = (isset(USUARIOS_NIVEL_INT[$this->nivel]))?USUARIOS_NIVEL_INT[$this->nivel]:0;
		$this->tsession = ((isset($this->opciones->tsession))? $this->opciones->tsession:3600);
		// if (!is_array($this->perfil_id))
		// $this->perfil_id = json_decode($this->perfil_id);
	}
...
* @return bool
*/
	public function SetNewPassword($password, $usuario_id = 1) {
		$result = false;
		try {
