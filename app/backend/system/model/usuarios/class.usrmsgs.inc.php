...
*/
	public function Get():?object {
		...
		try {
			...
			$this->sql = "SELECT * FROM ".SQLQuote($this->tabla_mensajes)." AS `mensajes` WHERE `mensajes`.`id` = ".$this->id." AND ((FIND_IN_SET('".$this->usuario->nivel."', `mensajes`.`grupo`)) OR (`mensajes`.`to_id` = ".$this->usuario->id.") OR (`mensajes`.`from_id` = ".$this->usuario->id.") OR (FIND_IN_SET('ALL', `mensajes`.`grupo`)))";
