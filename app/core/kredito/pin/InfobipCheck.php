<?php
/**
	Esto es lo que lidia con InfoBip para enviar el PIN.
	
	Las entradas son $pin y $pinid
...
if ($sysParams->Get('infobip_modo_test', false)) {
	...
	$infobip_response["response"] = ["pinId" => $pinid, "pin" => $pin, "verified" => true];
...
$infobip_response = $infobip->VerificarPin($pin, $pinid);
if (!$infobip_response) {
	cLogging::Write(__FILE__ ." ".__LINE__ ." cInfobipSms: ".$infobip->errmsg);
	$continue = $ws->SendResponse(500, 'PIN no pudo ser verificado.');
	return;
}
