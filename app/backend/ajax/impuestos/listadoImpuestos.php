<?php 
	...
	$result = $impuesto->List("IMP",$params);
	$hlp = new ListHelper;
	$hlp->SetPropeties($impuesto);
	$hlp->ListadoMgr = 'listadoImpuestos';
?>
<span class="lid" id="lid" hidden><?php echo $impuesto->listid ?? null; ?></span>
	<table class="tabla-general table table-striped table-hover mb-0 d-block d-lg-table table-responsive" id="listaImpuesto">
...
if(CanUseArray($result)){
	...
	?>
			<tr ondblclick="IsEdit('IMP',<?php echo $value->id; ?>)">
				...
				<td class="text-center"><button type="button" class="btn btn-primary btn-sm" title="Editar Impuestos" data-bs-target="#carouselImpuestos" data-bs-slide-to="1" aria-label="Slide 2"  onclick="IsEdit('IMP',<?php echo $value->id; ?>)"><i class="fas fa-user-edit me-2"></i>Editar</button></td>
			</tr>
	<?php
	}
}else{ ?>
