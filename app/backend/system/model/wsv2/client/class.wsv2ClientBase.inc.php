...
class cWsV2ClientBase extends cV2Base {
	...
	public $BackendId = null; // ID del usuario del backend actualmente en sesión.
...
*/
	public function Commit($method = '', $uri = '', $params = null) {
		...
		try {
			...
			if (!empty($this->BackendId)) {
				$this->AddHTTPHeader('Backend-id',$this->BackendId);
			}
			...
			if ($this->ExecRequest()) {
				$this->ParseResponse();
				$result = (isset($this->theData))?$this->theData:true;
				$this->core_nroerr = -1;
...
*/
	public function GetToken():bool {
		...
		try {
			...
			$this->ParseResponse();
			if (isset($this->theData) and isset($this->theData->token)) {
			...
			if (isset($this->theData) and isset($this->theData->expire)) {
...
*/	
	protected function SetHTTPError() {
		...
		$this->http_status = $this->http_nroerr;
...
/**
* Summary. Establece el ID del usuario del backend actualmente en sesión. Necesario para mantener las opciones del usuario actual en el core.
* @param mixed $backendUser instancia de la clase cUsrBackend
*/
	public function SetBackendUser(cUsrBackend $backendUser = null) {
		global $objeto_usuario;

		$this->acceptCookies = false;
		$this->BackendId = null;

		if (empty($backendUser)) { $backendUser = $objeto_usuario??null; }
		if (empty($backendUser)) { return; }
		if (!is_object($backendUser)) { return; }
		if (!is_a($backendUser, "cUsrBackend")) { return; }
		if (!$backendUser->existe) { return; }
		if (!$backendUser->esta_logueado) { return; }

		$this->BackendId = $backendUser->id;
		$this->SetTempDir($backendUser->username);
		$this->acceptCookies = true;
		$this->cookiesjar = $this->GetUserCookies();
	}
/**
* Summary. Establece las cookie. Esto está enlazado al usuario del backend actualmente loggeado.
*/
	private function SetCookies() {
		if ($this->acceptCookies and !empty($this->cookiesjar)) {
			$this->cURLOptions[CURLOPT_COOKIESESSION] = true;
			$this->cURLOptions[CURLOPT_COOKIEJAR] = $this->cookiesjar;
			$this->cURLOptions[CURLOPT_COOKIEFILE] = $this->cookiesjar;
		}
	}
} // Class.
