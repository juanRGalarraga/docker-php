...
try {
	...
	if (isset(ASSETS_CONTENTS[$handler[0]])) {
		...
		array_shift($handler);
		if (strtolower($handler[0]??null) == strtolower(DEFAULT_FOR_FILES)) {
			$for_files = true;
			array_shift($handler);
		}
	}
	...
	if (!$asset) {
		if ((INTERFACE_TYPE == 'backend') and ($objeto_contenido->esta_protegido != 0) and (!$user_logged_in) and (!$for_files)) {
			$objeto_contenido->GetContent('login');
		}
		
		if (isset($objeto_contenido->controlador)) {
