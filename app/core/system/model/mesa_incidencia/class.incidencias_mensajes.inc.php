...
class cIncidenciasMensajes extends cModels {

	const tabla_mensajes = TBL_mensajes_incidencias;
	public $qMainTable = TBL_mensajes_incidencias;
	public $usuario_id = null;
	public function __construct() {
		parent::__construct();
		$this->mainTable = self::tabla_mensajes;
    ...
    *   @return object $result El registro obtenido
	*/
	public function GetByIncidencia($indicencia) {
		$result = false;
		try {
			if (!SecureInt($indicencia)) { throw new Exception(" No se indico el identificador de persona "); }
			$this->sql = "SELECT * FROM ".$this->qMainTable." WHERE `incidencia_id` = ".$indicencia;
