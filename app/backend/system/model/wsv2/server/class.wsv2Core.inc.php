...
require_once(DIR_wsserver."class.wsv2Users.inc.php");
...
* @return string/bool $result El path absoluto al archivo de resolución o false en caso que no exista.
*/	
	public function GetResolver(array $contenido):?string {
		$result = false;
		$this->theResolver = '';
		...
		try {
			...
			if (!empty($contenido['resolver'])) {
				$this->theResolver = $baseDir.$contenido['resolver'].'.php';
				if (!ExisteArchivo($this->theResolver)) { throw new Exception('No se encontró archivo '.$this->theResolver); }
				$result = $this->theResolver;
			}
