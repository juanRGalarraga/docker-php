<?php
/*
	Inicio del Framework RebritGenerator 1.8
	Author: DriverOp.
	Created: 2021-02-05
*/
...
// if (!ExisteArchivo(DIR_model.'class.dbutili.2.inc.php')) { die("No puedo acceder a la clase de acceso a base de datos dbutili.2.inc.php"); }
// require_once(DIR_model . 'class.dbutili.2.inc.php'); // Clase base para el manejo de la base de datos.
if (!ExisteArchivo(DIR_model.'class.dbutil.3.0.inc.php')) { die("No puedo acceder a la clase de acceso a base de class.dbutil.3.0.inc.php"); }
...
$required_classes = [DIR_includes . "class.security.inc.php"];
...
$for_dirs = false; // Determina si se carga un archivo asset directamente a través de una ruta.
...
try {
	...
	if (isset(ASSETS_CONTENTS[$handler[0]])) {
		...
		if (strtolower(@$handler[0]) == strtolower(DEFAULT_FOR_DIRS)) {
			$for_dirs = true;
			array_shift($handler);
		}
	}

	if (!$for_files and !$for_dirs and (count($handler) > 0)) { // Si no se pide un archivo directamente...
	...
	if ($asset) {
		...
		if ((INTERFACE_TYPE == 'backend') and ($objeto_contenido->esta_protegido) and (!$user_logged_in) and !$for_files and !$for_dirs) {
	...
	cLogging::Write(__FILE__." ".__LINE__." Error cacheado: ".$e->GetMessage());
} finally {
