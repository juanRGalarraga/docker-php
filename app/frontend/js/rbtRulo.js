...
var ruloStyle = document.createElement('style');
ruloStyle.setAttribute('media','screen');
ruloStyle.setAttribute('type','text/css');
ruloStyle.setAttribute('id','rbtRuloStyles');
ruloStyle.appendChild(document.createTextNode(""));
ruloStyle.innerHTML = `div.rbtRuloVail{display:block;position:absolute;width:100%;height:100%;top:0;left:0;background-color:rgba(0,0,0,0.5);z-index:5000;}div.rbtRuloMain {position:absolute;background-position:center center;background-repeat:no-repeat;z-index:5001;padding:5px;min-width:25px;min-height:25px;/*background-color:white;border-radius:3px;border:1px solid #ccc;box-shadow: 10px 10px 10px #333;*/}div.rbtRuloMain>img{display:block;margin:auto;max-width:100px;}div>p.rbtRuloMsg{font-size:10px;color:#333;font-weight:normal;line-height:12px;margin:3px 0 0 0;text-align: center;}`;
document.head.appendChild(ruloStyle);
...
var rbtRulo = function (param) {
	this.defaultOptions = {
		...
		id: '01',
		message: '',
		animateImage: 'imgs/rulo_tenela_transparente.gif'
	}
...
*/
	this.Show = function (selector, message) {
		...
		if (TheRulo.theContainer == false) { return false; }
