...
class cContenido extends cModels {
	...
	public $breadcrumbs_id = array(); // Esto almacena la lista de IDs de los ancestros del contenido actual incluyendo el propio.
    private $tabla_permisos = "";
	...
	public function __construct() {
		...
		$this->tabla_permisos = SQLQuote(TBL_backend_usuarios_permisos);
		$this->ResetInstance();
...
*/
	public function GetMenuItems($parent = 0, $protegidos = false, $enmenu = true)	{
		...
		try {
			$sql = "SELECT `id`, `alias`, `nombre`, `metadata`, `es_default` FROM $this->qMainTable AS `contenidos` ";

			$where = "WHERE 1=1 ";
			if (isset($objeto_usuario) and !empty($objeto_usuario) and is_object($objeto_usuario) and ($objeto_usuario->existe)) { 
				// Hay un usuario por el cual preguntar?
				if (!$this->usuario->nivel !== 'OWN') { // No es owner?.
					$where = " `contenidos`.`permit`= true "; 
				}
			}
			$sql .= $where;
