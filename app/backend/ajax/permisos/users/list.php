...
try{
    ...
    if (isset($_SESSION['usuariosListadoPermisos'])){
        $ses = $_SESSION['usuariosListadoPermisos'];
    }
    ...
    $gen->where = " `people`.`negocio_id` = ".$objeto_usuario->negocio_id." AND `user`.`estado` LIKE '$estado'";
    $gen->order = array(
        array(
            "DESC" => "`people`.`id`"
        )
        ...
        <div class="card-body px-0">
            <table class="tabla-general table table-striped table-hover mb-0 d-block d-md-table table-responsive" id="listaVendedores">
                <thead class="thead-light">
                    <tr>
                        <th data-field="0" title="Nombre completo">Nombre y Apellido</th>
                        <th data-field="1" title="N° de Documento">Numero de Documento</th>
                        <th data-field="2" title="Nivel del cargo">Cargo</th>
                        <th data-field="2" title="Estado Cuenta">Estado</th>
                        <th></th>
                    </tr>
                </thead>
...
?>
                    <tr ondblclick="GetUserTemplate('<?php echo $value->people_id?>')">
                        ...
                        <td class="text-center"><button type="button" class="btn btn-primary btn-sm" title="Editar plantilla" data-bs-target="#carouselExampleControls" data-bs-slide-to="1" aria-label="Slide 2"  onclick="GetUserTemplate('<?php echo $value->people_id?>')"><i class="fas fa-tasks-alt"></i> Editar plantilla</button></td>
                    </tr>
<?php
        }
?>  
